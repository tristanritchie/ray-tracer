...
public:
  ...
  rtMatrix2();
  rtMatrix2(int nRows, int nCols);
  rtMatrix2(int nRows, int nCols, const T* inputData);
  rtMatrix2(const rtMatrix2<T>& inputMatrix);
  rtMatrix2(int nRows, int nCols, const std::vector<T>& inputData);
  ...
  rtMatrix2<T> RowEchelon();
  // Return the transpose.
  rtMatrix2<T> Transpose() const;

  // Compute determinant.
  T Determinant();

  // Overload == operator.
  bool operator==(const rtMatrix2<T>& rhs);
  bool Compare(const rtMatrix2<T>& matrix1, double tolerance);

  // Overload the assignment operator.
  rtMatrix2<T> operator=(const rtMatrix2<T>& rhs);

  // Overload +, - and * operators (friends).
  template <class U>
  friend rtMatrix2<U> operator+(const rtMatrix2<U>& lhs,
                                const rtMatrix2<U>& rhs);
  template <class U>
  friend rtMatrix2<U> operator+(const U& lhs, const rtMatrix2<U>& rhs);
  template <class U>
  friend rtMatrix2<U> operator+(const rtMatrix2<U>& lhs, const U& rhs);
  ...
  // rtMatrix2 * rtVector.
  template <class U>
  friend rtVector<U> operator*(const rtMatrix2<U>& lhs, const rtVector<U>& rhs);

  bool Separate(rtMatrix2<T>& matrix1, rtMatrix2<T>& matrix2, int colNum);
  bool Join(const rtMatrix2<T>& matrix2);
  rtMatrix2<T> FindSubMatrix(int rowNum, int colNum);
...
template <class T>
rtMatrix2<T>::rtMatrix2(int nRows, int nCols)
{
  m_nRows = nRows;
  m_nCols = nCols;
...
template <class T>
rtMatrix2<T>::rtMatrix2(int nRows, int nCols, const T* inputData)
{
  m_nRows = nRows;
  m_nCols = nCols;
  m_nElements = m_nRows * m_nCols;
...
template <class T>
rtMatrix2<T>::rtMatrix2(const rtMatrix2<T>& inputMatrix)
{
...
template <class T>
rtMatrix2<T>::rtMatrix2(int nRows, int nCols, const std::vector<T>& inputData)
{
  m_nRows = nRows;
  m_nCols = nCols;
  m_nElements = m_nRows * m_nCols;
...
template <class T>
bool rtMatrix2<T>::Resize(int numRows, int numCols)
{
  m_nRows = numRows;
  m_nCols = numCols;
  m_nElements = (m_nRows * m_nCols);
...
template <class T>
void rtMatrix2<T>::SetToIdentity()
{
  if (!IsSquare())
    throw std::invalid_argument(
        "Cannot form an identity matrix that is not square.");
...
template <class T>
T rtMatrix2<T>::GetElement(int row, int col) const
{
...
template <class T>
bool rtMatrix2<T>::SetElement(int row, int col, T elementValue)
{
  int linearIndex = Sub2Ind(row, col);
  if (linearIndex >= 0)
  {
...
template <class T>
int rtMatrix2<T>::GetNumRows() const
{
  return m_nRows;
}

template <class T>
int rtMatrix2<T>::GetNumCols() const
{
  return m_nCols;
}

template <class T>
bool rtMatrix2<T>::Compare(const rtMatrix2<T>& matrix1, double tolerance)
{
...
// matrix + matrx.
template <class T>
rtMatrix2<T> operator+(const rtMatrix2<T>& lhs, const rtMatrix2<T>& rhs)
{
  ...
  rtMatrix2<T> result(numRows, numCols, tempResult);
  delete[] tempResult;
  return result;
}
...
// scaler + matrix
template <class T>
rtMatrix2<T> operator+(const T& lhs, const rtMatrix2<T>& rhs)
{
...
// matrix + scaler
template <class T>
rtMatrix2<T> operator+(const rtMatrix2<T>& lhs, const T& rhs)
{
  int numRows = lhs.m_nRows;
  int numCols = lhs.m_nCols;
  int numElements = numRows * numCols;
...
rtMatrix2<T> operator-(const rtMatrix2<T>& lhs, const rtMatrix2<T>& rhs)
{
  ...
  rtMatrix2 result(numRows, numCols, tempResult);
  delete[] tempResult;
  return result;
}
...
// matrix * vector
template <class T>
rtVector<T> operator*(const rtMatrix2<T>& lhs, const rtVector<T>& rhs)
{
  ...
  rtVector<T> result(lhs.m_nRows);

  // Loop over rows and columns and perform the multiplication operation
  // element-by-element.
...
rtMatrix2<T> operator*(const rtMatrix2<T>& lhs, const rtMatrix2<T>& rhs)
{
  ...
  if (l_numCols == r_numRows)
  {
    ...
    for (int lhsRow = 0; lhsRow < l_numRows; lhsRow++)
    {
      ...
      for (int rhsCol = 0; rhsCol < r_numCols; rhsCol++)
      {
        ...
        for (int lhsCol = 0; lhsCol < l_numCols; lhsCol++)
        {
          ...
          elementResult += (lhs.m_matrixData[lhsLinearIndex] *
                            rhs.m_matrixData[rhsLinearIndex]);
        }
    ...
    rtMatrix2<T> result(l_numRows, r_numCols, tempResult);
    delete[] tempResult;
    return result;
  }
  else
  {
    rtMatrix2<T> result(1, 1);
    return result;
  }
}

template <class T>
bool rtMatrix2<T>::operator==(const rtMatrix2<T>& rhs)
{
...
template <class T>
rtMatrix2<T> rtMatrix2<T>::operator=(const rtMatrix2<T>& rhs)
{
...
template <class T>
bool rtMatrix2<T>::Separate(rtMatrix2<T>& matrix1, rtMatrix2<T>& matrix2,
                            int colNum)
{
  ...
  // Loop over the original matrix and store data into the appropriate elements
...
template <class T>
bool rtMatrix2<T>::Join(const rtMatrix2<T>& matrix2)
{
  ...
  // If the matrices have different numbers of rows, then this operation makes
  // no sense.
  if (numRows1 != numRows2)
    throw std::invalid_argument(
        "Attempt to join matrices with different numbers of rows is invalid.");
  ...
  for (int i = 0; i < numRows1; ++i)
  {
    for (int j = 0; j < (numCols1 + numCols2); ++j)
    {
      ...
      // Otherwise, j must be in the right hand matrix, so we get data from
      // there...
      else
      {
...
template <class T>
T rtMatrix2<T>::Determinant()
{
  // Check if the matrix is square.
  if (!IsSquare())
    throw std::invalid_argument(
        "Cannot compute the determinant of a matrix that is not square.");

  // If the matrix is 2x2, we can just compute the determinant directly.
  T determinant;
  if (m_nRows == 2)
  {
    determinant = (m_matrixData[0] * m_matrixData[3]) -
                  (m_matrixData[1] * m_matrixData[2]);
  }
  else
  {
    /* Otherwise we extract the sub-matrices and then recursively call this
       function until we get to 2x2 matrices. */
    ...
    for (int j = 0; j < m_nCols; ++j)
    {
      ...
      rtMatrix2<T> subMatrix = this->FindSubMatrix(0, j);
...
template <class T>
bool rtMatrix2<T>::Inverse()
{
  // Check if the matrix is square (we cannot compute the inverse if it isn't).
  if (!IsSquare())
    throw std::invalid_argument(
        "Cannot compute the inverse of a matrix that is not square.");

  // If we get to here, the matrix is square so we can continue.

  // Form an identity matrix with the same dimensions as the matrix we wish to
  // invert.
  rtMatrix2<T> identityMatrix(m_nRows, m_nCols);
  ...
  while ((!completeFlag) && (count < maxCount))
  {
    ...
    {
      // Loop over the diagonal of the matrix and ensure all diagonal elements
      // are equal to one.
      cRow = diagIndex;
      cCol = diagIndex;
      ...
      if (maxIndex != cRow)
      {
        // std::cout << "Swap rows " << cRow << " and " << maxIndex <<
        // std::endl;
      ...
      if (m_matrixData[Sub2Ind(cRow, cCol)] != 1.0)
      {
        ...
        // std::cout << "Multiply row " << cRow << " by " << multFactor <<
        // std::endl;
      }
      ...
      for (int colIndex = cCol + 1; colIndex < originalNumCols; ++colIndex)
      {
        ...
        if (!CloseEnough(m_matrixData[Sub2Ind(cRow, colIndex)], 0.0))
        {
          ...
          if (!CloseEnough(rowOneValue, 0.0))
          {
            ...
            // To make this equal to zero, we need to add -currentElementValue
            // multiplied by the row at rowOneIndex.
            MultAdd(cRow, rowOneIndex, correctionFactor);

            // std::cout << "Multiply row " << rowOneIndex << " by " <<
            // correctionFactor << 	" and add to row " << cRow << std::endl;
          }
        }
      }
    }
    ...
    rtMatrix2<T> leftHalf;
    rtMatrix2<T> rightHalf;
    this->Separate(leftHalf, rightHalf, originalNumCols);

    // When the process is complete, the left half should be the identity
    // matrix.
    if (leftHalf == identityMatrix)
    {
      ...
      // Rebuild the matrix with just the right half, which now contains the
      // result.
...
template <class T>
rtMatrix2<T> rtMatrix2<T>::Transpose() const
{
  // Form the output matrix.
  // Note that we reverse the order of rows and columns, as this will be the
  // transpose.
  rtMatrix2<T> resultMatrix(m_nCols, m_nRows);
...
template <class T>
rtMatrix2<T> rtMatrix2<T>::RowEchelon()
{
  /* The current matrix must have at least as many columns as rows, but note
     that we don't actually require it to be square since we assume that the
     user may have combined a square matrix with a vector. They would do this,
     for example, if they were trying to solve a system of linear equations. */
  if (m_nCols < m_nRows)
    throw std::invalid_argument(
        "The matrix must have at least as many columns as rows.");

  /* Make a copy of the matrix data before we start. We do this because the
     procedure below will make changes to the stored matrix data (it operates
     'in place') and we don't want this behaviour. Therefore we take a copy at
     the beginning and then we will replace the modified matrix data with this
     copied data at the end, thus preserving the original. */
  T* tempMatrixData;
  ...
  // Form the output matrix.
  rtMatrix2<T> outputMatrix(m_nRows, m_nCols, m_matrixData);
...
template <class T>
int rtMatrix2<T>::Rank()
{
  // Convert the matrix to row-echelon form.
  rtMatrix2<T> matrixCopy = this->RowEchelon();
  ...
  if (!matrixCopy.IsRowEchelon())
  {
    ...
    std::vector<rtMatrix2<T>> subMatrixVector;
    ...
    while ((subMatrixCount < subMatrixVector.size()) && (!completeFlag))
    {
      ...
      rtMatrix2<T> currentMatrix = subMatrixVector[subMatrixCount];
      subMatrixCount++;
      ...
      if (currentMatrix.IsNonZero())
      {
        ...
        else
        {
          ...
          if ((currentMatrix.GetNumRows() > 2) &&
              (currentMatrix.GetNumCols() > 2))
          {
...
// Function to return the linear index corresponding to the supplied row and
// column values.
template <class T>
int rtMatrix2<T>::Sub2Ind(int row, int col) const
{
...
template <class T>
bool rtMatrix2<T>::IsRowEchelon()
{
  /* We do this by testing that the sum of all the elements in the
          lower triangular matrix is zero. */
  // Loop over each row, except the first one (which doesn't need to have any
  // zero elements).
  T cumulativeSum = static_cast<T>(0.0);
...
template <class T>
void rtMatrix2<T>::SwapRow(int i, int j)
{
...
template <class T>
void rtMatrix2<T>::MultAdd(int i, int j, T multFactor)
{
  for (int k = 0; k < m_nCols; ++k)
...
template <class T>
int rtMatrix2<T>::FindRowWithMaxElement(int colNumber, int startingRow)
{
...
template <class T>
void rtMatrix2<T>::MultRow(int i, T multFactor)
{
  for (int k = 0; k < m_nCols; ++k)
...
template <class T>
void rtMatrix2<T>::PrintMatrix(int precision)
{
  int nRows = this->GetNumRows();
  int nCols = this->GetNumCols();
  for (int row = 0; row < nRows; ++row)
...
template <class T>
bool rtMatrix2<T>::CloseEnough(T f1, T f2)
{
...
template <class T>
rtMatrix2<T> rtMatrix2<T>::FindSubMatrix(int rowNum, int colNum)
{
  // Create a new matrix to store the sub-matrix.
  // Note that this is one row and one column smaller than the original.
  rtMatrix2<T> subMatrix(m_nRows - 1, m_nCols - 1);

  // Loop over the elements of the existing matrix and copy to sub-matrix as
  // appropriate.
  int count = 0;
  for (int i = 0; i < m_nRows; ++i)
  {
