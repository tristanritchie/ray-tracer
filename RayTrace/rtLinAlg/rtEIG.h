...
template <typename T>
int qbEigQR(const rtMatrix2<T>& inputMatrix, std::vector<T>& eigenValues)
{
  // Make a copy of the input matrix.
  rtMatrix2<T> A = inputMatrix;
  ...
  rtMatrix2<T> identityMatrix(numRows, numRows);
  identityMatrix.SetToIdentity();

  // Create matrices to store Q and R.
  rtMatrix2<T> Q(numRows, numRows);
  rtMatrix2<T> R(numRows, numRows);
...
template <typename T>
int qbInvPIt(const rtMatrix2<T>& inputMatrix, const T& eigenValue,
             rtVector<T>& eigenVector)
{
  // Make a copy of the input matrix.
  rtMatrix2<T> A = inputMatrix;
  ...
  rtVector<T> prevVector(numRows);
  rtMatrix2<T> tempMatrix(numRows, numRows);
...
// The rtEig function (power iteration method).
template <typename T>
int qbEIG_PIt(const rtMatrix2<T>& X, T& eigenValue, rtVector<T>& eigenVector)
{
  // Make a copy of the input matrix.
  rtMatrix2<T> inputMatrix = X;
  ...
  ************************************************************** */
  ...
  eigenValue =
      (cumulativeSum / v1.GetElement(0)) + inputMatrix.GetElement(0, 0);
