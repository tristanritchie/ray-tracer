...
rtVector<double> RT::MaterialBase::ComputeColor(
...
rtVector<double> RT::MaterialBase::ComputeDiffuseColor(
    const std::vector<std::shared_ptr<RT::ObjectBase>>& objectList,
    const std::vector<std::shared_ptr<RT::LightBase>>& lightList,
    const std::shared_ptr<RT::ObjectBase>& currentObject,
    const rtVector<double>& intPoint, const rtVector<double>& localNormal,
    const rtVector<double>& baseColor)
{
  ...
  rtVector<double> diffuseColor{3};
  double intensity;
  rtVector<double> color{3};
  double red = 0.0;
  double green = 0.0;
  double blue = 0.0;
...
rtVector<double> RT::MaterialBase::ComputeReflectionColor(
    const std::vector<std::shared_ptr<RT::ObjectBase>>& objectList,
    const std::vector<std::shared_ptr<RT::LightBase>>& lightList,
    const std::shared_ptr<RT::ObjectBase>& currentObject,
    const rtVector<double>& intPoint, const rtVector<double>& localNormal,
...
{
  rtVector<double> reflectionColor{3};

  // Compute the reflection vector.
  rtVector<double> d = incidentRay.m_lab;
  rtVector<double> reflectionVector =
...
bool RT::MaterialBase::CastRay(
    ...
    std::shared_ptr<RT::ObjectBase>& closestObject,
    rtVector<double>& closestIntPoint, rtVector<double>& closestLocalNormal,
