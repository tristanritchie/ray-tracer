...
rtVector<double> RT::Texture::Image::GetColor(const rtVector<double>& uvCoords)
{
  ...
  if (!m_imageLoaded)
  {
    ...
    outputColor = rtVector<double>{std::vector<double>{1.0, 0.0, 1.0, 1.0}};
  }
  else
  {
    ...
    int x = static_cast<int>(
        round(((u + 1.0) / 2.0) * static_cast<double>(m_xSize)));
    int y = m_ySize - (static_cast<int>(round(((v + 1.0) / 2.0) *
                                              static_cast<double>(m_ySize))));
...
bool RT::Texture::Image::LoadImage(std::string fileName)
{
  ...
  std::cout << "Loaded " << m_imageSurface->w << " by " << m_imageSurface->h
            << "." << std::endl;
